...
class ReservationsSearchForm(forms.Form):
  ...
  sort_by = forms.ChoiceField(required=False, choices=(('', '---------'),
                                                       ...
                                                       ('status', 'Status')))
  columns = forms.MultipleChoiceField(required=False, choices=models.RESERVATION_TABLE_COLUMN_CHOICES, initial=['CR', 'UR', 'KT', 'CO', 'EQ', 'CC', 'DA', 'DD', 'RD', 'AN', 'HP', 'AT', 'ST', 'GG'],  widget=forms.SelectMultiple(attrs={'size':6}), label=u'Display Columns')
  rows_per_page = forms.ChoiceField(required=True, choices=models.TABLE_ROWS_PER_PAGE_CHOICES, initial=25)
